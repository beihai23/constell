...
| 搜索引擎 | Meilisearch / Elasticsearch | 消息搜索、用户搜索 |
...
| **Community Service** | 有状态 (groupcache) | channel_id | Server/Guild CRUD，Channel CRUD，成员管理，角色/权限模型，**群聊消息存储和历史**，已读状态/Reaction/Pin |
| **File Service** | 无状态 | — | 文件上传/下载，图片缩略图，对象存储管理 |
| **Search Service** | 无状态 | — | 消息搜索，用户搜索。订阅 NATS 事件异步建索引 |
