...
| 全文搜索 | PostgreSQL tsvector/tsquery | 初期不引入独立搜索引擎，用 PG 原生全文搜索 |
...
| **User Service** | 有状态 (groupcache) | user_id | 用户 Profile，好友关系/黑名单，**DM 对话和消息**，在线状态查询，用户搜索 |
| **Community Service** | 有状态 (groupcache) | channel_id | Server/Guild CRUD，Channel CRUD，成员管理，角色/权限模型，**群聊消息存储和历史**，已读状态/Reaction/Pin，消息搜索 |
...
1. **一致性哈希分区**：每个实例"拥有"一部分数据
   ...
   - Community Service：按 channel_id 分区，owning node 缓存该频道的元数据、成员列表、权限定义
...
- **用户搜索**：通过 PostgreSQL `tsvector` 全文搜索用户昵称
...
- **消息搜索**：通过 PostgreSQL `tsvector` 全文搜索频道内消息内容
...
- PG 全文搜索 → Meilisearch/Elasticsearch（搜索量或复杂度超出 PG 能力时）
- 添加语音/视频通话
- Bot/集成框架
